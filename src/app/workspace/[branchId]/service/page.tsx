...
export default async function ServicePage({ params }: { params: Promise<{ branchId: string }> }) {
  ...
  return <>
    ...
    <ServiceConsole branchId={branch.id} currency={branch.currency} origin={origin} snapshot={snapshot} canBill={hasPermission(branch.role, "billing:manage")} canQueue={hasPermission(branch.role, "queue:manage")} locale={locale}/>
  </>;
}
