...
export function Workspace() {
  ...
  function publish() {
    if (!draft) return;
    const overlap = draft.tables.some((a, i) => draft.tables.slice(i + 1).some(b => Math.abs(a.x - b.x) < 15 && Math.abs(a.y - b.y) < 22));
    const blocked = draft.tables.some(t => draft.blocks.some(b => t.x < b.x + b.width && t.x + 15 > b.x && t.y < b.y + b.height && t.y + 18 > b.y));
  ...
  function move(clientX: number, clientY: number) {
    ...
    const maxX = 100 - (object?.width ?? 17), maxY = 100 - (object?.height ?? 24);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <a className="brand" href="/" aria-label="Tablejoy home">table<span>joy</span><span className="brand-dot">✳</span></a>
    ...
    <main className="main">
      ...
      <div className={`workspace-grid ${view === "queue" ? "queue-focus" : ""}`}>
        ...
        {editing && <div className="editor-toolbar"><label>Seats <input aria-label="New table capacity" type="number" min={1} max={20} value={capacity} onChange={e => setCapacity(Math.max(1, Math.min(20, Number(e.target.value) || 1)))}/></label><Button variant="outline" onClick={addTable}><Plus/>Table</Button><Button variant="outline" onClick={() => edit(b => ({ ...b, blocks: [...b.blocks, { id: crypto.randomUUID(), label: "WALL", x: 5, y: 5, width: 22, height: 5 }] }))}><Square/>Block</Button><Button variant="ghost" aria-label="Undo layout change" disabled={!history.length} onClick={() => { setDraft(history[history.length - 1]); setHistory(h => h.slice(0, -1)); }}><RotateCcw/></Button><Button onClick={publish}><Save/>Publish</Button></div>}
        <div className={`floor-canvas ${editing ? "editing" : ""}`} ref={canvasRef} onPointerMove={event => move(event.clientX, event.clientY)} onPointerUp={() => { dragRef.current = null; }} onPointerCancel={() => { dragRef.current = null; }}>
        ...
        <div className="floor-footer"><div className="legend"><span><i className="available"/>Available</span><span><i className="occupied"/>Occupied</span><span><i className="reserved"/>Reserved</span></div><span className="floor-hint"><MousePointer2 size={14}/>{editing ? "Drag to arrange" : "Select a table"}</span></div>
        {table && <div className="table-detail"><div><strong>Table {table.label}</strong><span>{table.capacity} seats · {table.status}{table.accessible ? " · Accessible" : ""}</span></div>{editing ? <div className="position-fields"><label>X <input type="number" min={2} max={83} value={table.x} disabled={table.status !== "available"} onChange={e => edit(b => ({ ...b, tables: b.tables.map(t => t.id === table.id ? { ...t, x: Math.max(2, Math.min(83, Number(e.target.value))) } : t) }))}/></label><label>Y <input type="number" min={2} max={76} value={table.y} disabled={table.status !== "available"} onChange={e => edit(b => ({ ...b, tables: b.tables.map(t => t.id === table.id ? { ...t, y: Math.max(2, Math.min(76, Number(e.target.value))) } : t) }))}/></label><Button variant="ghost" disabled={table.status !== "available"} onClick={() => { edit(b => ({ ...b, tables: b.tables.filter(t => t.id !== table.id) })); setSelected(""); }}>Remove</Button></div> : <Button variant="outline" disabled={table.status === "available"} onClick={() => { update(b => ({ ...b, tables: b.tables.map(t => t.id === table.id ? { ...t, status: "available" } : t) })); notify(`Table ${table.label} is now available in the demo.`); }}><Check/>Mark available</Button>}</div>}
        </section>
        <aside className="queue-panel"><div className="queue-title"><div><h2>A seat for everyone</h2><span>{waiting.length} parties in line</span></div><span className="sparkle-tile"><Sparkles size={19}/></span></div>
        ...
        {editing && <div className="recommendation"><h3>Your floor, reimagined.</h3><p>Add tables with any capacity, move blocks, and publish when ready. Occupied and reserved tables stay in place.</p></div>}
      ...
      <div className="bottom-note"><span>Made for the little moments between “welcome” and “see you soon”.</span><span>tablejoy ✳</span></div>
      </>}
