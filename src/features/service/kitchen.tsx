...
export function KitchenBoard({ branchId, tickets }: Props) {
  ...
  function move(orderId: string, status: "preparing" | "served" | "cancelled") {
    startTransition(async () => {
      ...
      } catch { setIsError(true); setMessage("Connection interrupted. Refresh to see the current board."); }
    });
  }
  ...
  return <section className="kitchen">
    ...
    {tickets.length === 0
      ? <p className="pos-empty"><ChefHat size={30}/><br/>No tickets cooking. The board updates on its own.</p>
          ...
          return <article key={ticket.id} className={`ticket ${ticket.status} ${minutes >= 20 ? "late" : ""}`}>
            ...
            <footer>
              {ticket.status === "placed"
                ? <Button disabled={pending} onClick={() => move(ticket.id, "preparing")}><ChefHat/>Start cooking</Button>
